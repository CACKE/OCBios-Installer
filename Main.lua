...
local localizations = {
    ...
    ["Russian"] = {
        flash = "Установка прошивки",
        reboot = "Ребут",
        downloading = "качаем чота...",
        flashing = "Прошиваем...",
        done = "Готово!",
        users = "Юзеры",
        ...
        readonly = "EEPROM только для котов:",
        userInput = "Ждать нажатия для загрузки:",
        options = "Настройки",
        flashFailed = "Прошив неудачен, причина:",
        downloadFailed = "Файл не скачался, хз почему:"
    }
}
...
local workspace, window = system.addWindow(GUI.titledWindow(1, 1, 52, 17, "OCBios Installer", true))
...
container:addChild(GUI.roundedButton(18, 15, 15, 1, 0xE1E1E1, 0x696969, 0x878787, 0xFFFFFF, localization.flash)).onTouch = function()
    ...
    if #config > 64 then
      ...
      if e1 == "touch" then
        ...
        workspace:draw()
        local data, reason = internet.request("https://pastebin.com/raw/AVhfEqN0")
        downloading:remove()
